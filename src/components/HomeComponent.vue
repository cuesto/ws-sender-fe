...
<v-layout align-start>
    <v-flex>
        <v-data-table :headers="headers" :search="search" :items="items" class="elevation-1" :items-per-page="10" :footer-props="{ 'items-per-page-options': [10, 50, 100, 250, 500] }">
            <template v-slot:top>
                <v-toolbar flat color="white">
                    <v-toolbar-title>Clientes</v-toolbar-title>
                    <v-divider class="mx-4" inset vertical></v-divider>
                    <v-btn :loading="loadingUploadBtn" :disabled="disableUploadBtn" color="blue" class="ma-2 white--text" @click="showUploadModal">
                        Cargar
                        <v-icon right dark> mdi-cloud-upload </v-icon>
                    </v-btn>
                    <v-tooltip top>
                        <template v-slot:activator="{ on, attrs }">
                            <v-btn class="ma-2" v-bind="attrs" v-on="on" href="template.csv" dark download>
                    ...
                    <v-spacer></v-spacer>
                    <v-btn :loading="loadingSendSingleBtn" :disabled="disableSendSingleBtn" color="orange" class="ma-2 white--text" @click="showMessageSingleModal">
                        Mensaje Único
                        <v-icon right dark>mdi-test-tube</v-icon>
                    </v-btn>
                    <v-tooltip top>
                        <template v-slot:activator="{ on, attrs }">
                            <v-btn v-bind="attrs" v-on="on" :loading="loadingSendBtn" :disabled="disableSendBtn" color="green" class="ma-2 white--text" @click="showMessageModal">
                                Mensaje Masivo
            ...
            <template #[`item.actions`]="{ item }">
                <v-icon size="sm" variant="outline-info" color="green" class="mr-1" v-if="item.isSent">done</v-icon>
                <v-icon size="sm" variant="outline-info" color="red" class="mr-1" v-if="item.hasError">mdi-alert-circle-outline</v-icon>
            </template>
        </v-data-table>
    </v-flex>
    ...
    <v-dialog v-model="messageModal" persistent max-width="600px">
        <v-form ref="form">
            <v-card>
                    ...
                    <v-container>
                        <v-row>
                            <v-col cols="12" sm="12" md="12">
                                <v-text-field v-if="showPhoneOnModal" label="Celular" hint="8094445555" v-mask="mask" :rules="[rules.required]" append-icon="mdi-phone" v-model="messageModel.phone" single-line></v-text-field>
                            </v-col>
                            <v-col cols="12" sm="12" md="12">
                                <v-textarea label="Mensaje" :hint="hintMessage" :rules="[rules.required]" append-icon="mdi-comment-text" v-model="messageModel.message"></v-textarea>
                            </v-col>
                        </v-row>
                    </v-container>
                </v-card-text>
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn color="blue darken-1" text @click="closeMessageModal()">Cerrar</v-btn>
